...
-- 初始化-部门表数据
-- ----------------------------
insert into sys_dept values(1,  0,   '0', '默认部门',  0, '', '', '', '0', '0', 'admin', sysdate(), '', null);
...
create table sys_user (
  ...
  status            char(1)         default '0'                comment '帐号状态（0在职 1离职 2禁用）',
  del_flag          char(1)         default '0'                comment '删除标志（0代表存在 2代表删除）',
  ...
  is_sale bit not null comment '是否是普通销售，普通销售必须在小组内，请假审批用到',
  sale_group_id varchar(32) comment '销售小组，一对一，会影响到审批',
  primary key (user_id)
...
-- 初始化-用户信息表数据
-- ----------------------------
insert into sys_user values(1,  1, 'admin', '管理员', '00', '0', '15888888888', 'test@qq.com', '', '$2a$10$7JB720yubVSZvUI0rEqK/.VqGOZTH.ulu33dHOiBE8ByOhJIrdAu2', '0', '0', '127.0.0.1', sysdate(), 'admin', sysdate(), '', null, '管理员账号', 18, null, '河南省', false, null);
...
-- 初始化-岗位信息表数据
-- ----------------------------
insert into sys_post values(1, 'default',  '默认岗位',    1, '0', 'admin', sysdate(), '', null, '');
...
create table sys_notice (
  ...
  notice_type       char(1)         not null                   comment '公告类型（1公告 2首页轮播图）',
...
) engine=innodb auto_increment=10 comment = '公告表';
